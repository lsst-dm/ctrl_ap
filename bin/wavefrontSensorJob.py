...
class WavefrontSensorJob(Job):

    def __init__(self, expectedVisitID, exposures)
    ...
    def getCameraImage(self, imageID, visitID, exposureSequenceID):
        self.logger(Log.INFO, "getting data for wave front ensors; visitID = %s" % (visitID)

if __name__ == "__main__":
    basename = os.path.basename(sys.argv[0])
    parser = argparse.ArgumentParser(prog=basename)
    parser.add_argument("-I", "--visitID", type=int, action="store", help="visit id", required=True)
    parser.add_argument("-n", "--exposures", type=int, action="store", help="number of exposures to wait for", required=True)

    args = parser.parse_args()
    base = WavefrontSensorJob(args.visitID, args.exposures)
    base.handleEvents()
